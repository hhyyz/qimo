...
public class ShoppingService {
	// 管理端查询详情
		public BaseDataPojo<Product> selectById(int id) {
			Connection conn = DButil.getConnection();
			ShoppingDao appIndexDao = new ShoppingDao(conn);
			try {
				Product appIndex = new Product();
				appIndex = appIndexDao.selectById(id); // 返回记录集
				conn.commit();
				return new BaseDataPojo<Product>("获取成功", true, appIndex);
			} catch (Exception e) {
				try {
					conn.rollback();
				...
				return new BaseDataPojo<Product>("获取失败", false, null);
			} finally {
				if (conn != null) {
	...
	public BaseDataPojo<ProductAll> selectApp(Product product) {
		Connection conn = DButil.getConnection();
		ShoppingDao shoppingDao = new ShoppingDao(conn);
		ArrayList<Product> list = new ArrayList<Product>();
		
		try {
			list = shoppingDao.select(product);
			
			conn.commit();
			ProductAll indexAll = new ProductAll();
			indexAll.setproduct(list);
			return new BaseDataPojo<ProductAll>("获取成功", true,indexAll);
			...
			return new BaseDataPojo<ProductAll>("获取失败", false,null);
		} finally {
			if (conn != null) {
				DButil.closeConnection(conn);
			}
